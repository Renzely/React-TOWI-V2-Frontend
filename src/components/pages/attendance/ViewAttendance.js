...
export default function ViewAttendance() {
  ...
  async function fetchAttendanceData(email, outlet, currentDate = new Date()) {
    ...
    try {
      const response = await axios.get(
        "https://towi-react.onrender.com/attendance/status",
        {
          params: {
            email: email,
            outlet: outlet,
  ...
  async function fetchAllAttendanceData(
    ...
    try {
      ...
      const response = await axios.post(
        "https://towi-react.onrender.com/get-attendance",
        requestBody
      );
  ...
  const getExportData = async () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "TOWI-Attendance");
      ...
      link.download = `TOWI_Attendance_${startDateStr}_to_${endDateStr}.xlsx`;
      ...
      <Box sx={{ display: "flex", flexDirection: { xs: "column", md: "row" } }}>
        ...
        <Box
          sx={{
            ...
            maxWidth: "100%",
            overflow: "auto",
            backgroundColor: "#f0f8ff", // Background color from attendance.js
          }}
        >
          <Box
            sx={{
